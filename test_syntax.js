...
async function syncLoadProductos() {
  ...
  try {
    ...
    rows.forEach(function(row) {
      var stk_alm = {};
      (row.stock_almacen || []).forEach(function(s) { stk_alm[s.almacen] = (stk_alm[s.almacen] || 0) + s.cantidad; });
      var stk = Object.values(stk_alm).reduce(function(a,v){return a+v;}, 0);
      var prod = {
        n: row.nombre, cat: row.categoria||'',
        stk_alm: stk_alm,
        lotes: (row.stock_almacen || []).map(function(s){
...
window.adjustStockByDelta = async function(prod, almacen, delta, preferredLote, opts) {
  ...
  if(isAdd) {
    ...
    if(!exist) body.fecha_entrada = new Date().toISOString();
        ...
        <div class="ci-badges">
          ...
          ${(function(){
            ...
            return '<select style="font-size:9px;padding:1px 3px;border:1px solid var(--color-border-secondary);border-radius:4px;background:var(--color-background-secondary);color:var(--color-text-secondary);max-width:90px" onchange="changeLoteCart('+i+',this.value)" title="Cambiar lote">'
              +_lotes.map(function(l){return '<option value="'+l.lote+'"'+(c.loteSeleccionado===l.lote?' selected':'')+'>'+l.lote+' ('+l.cantidad+')</option>';}).join('')
              +'</select>';
          })()}
  ...
  if(v.prods && typeof PRODS!=='undefined'){
    ...
    lineas.forEach(function(l){
      ...
      if(prod){
        ...
        // Persist to Supabase
        if(v.alm&&typeof adjustStockByDelta==='function'){
          if(prod.supaId){
            adjustStockByDelta(prod, v.alm, l.q);
          } else if(_supaOnline){
...
function registrarMovStock(data) {
  ...
  if (p && _supaOnline) {
    (async function(){
      ...
      if(typeof adjustStockByDelta === 'function') {
        var loteSelected = mov.contenedor || 'SIN LOTE';
        if(mov.tipo === 'traslado') {
          await adjustStockByDelta(p, mov.almacen, -mov.cantidad, loteSelected);
          await adjustStockByDelta(p, mov.almacenDest, mov.cantidad, loteSelected);
        } else if (mov.tipo === 'entrada' || mov.tipo === 'ajuste_alta') {
          await adjustStockByDelta(p, mov.almacen, mov.cantidad, loteSelected);
        } else if (mov.tipo === 'salida' || mov.tipo === 'ajuste_baja') {
          await adjustStockByDelta(p, mov.almacen, -mov.cantidad, loteSelected);
        } else if (mov.tipo === 'ajuste_abs') {
          // Adjust absolute by calculating diff before applyStockMov. Wait, applyStockMov already ran.
          // Since applyStockMov modified p.stk_alm, we shouldn't use it.
          // Better let adjustStockByDelta recalculate diff:
          var currentLotes = p.lotes ? p.lotes.filter(function(x){return x.almacen===mov.almacen;}).reduce(function(a,b){return a+(b.cantidad||0);},0) : 0;
          var diff = mov.cantidad - currentLotes;
          await adjustStockByDelta(p, mov.almacen, diff, loteSelected);
        }
...
window.openStockLotes = function(supaId) {
  ...
  window.renombrarLote = async function(idx, sId) {
    ...
    if(!nLote || nLote === l.lote) return;
    
    try {
      mo.style.pointerEvents = 'none'; mo.style.opacity = '0.5';
      
      var existLot = p.lotes.find(function(x){return x.almacen === l.almacen && x.lote === nLote;});
  ...
  var h = '<div style="background:var(--color-background-primary);padding:20px;border-radius:8px;width:90%;max-width:500px;max-height:90vh;overflow-y:auto;position:relative;color:var(--color-text-primary)">';
  ...
  if(lotes.length === 0) {
    ...
    h += '<table class="adm-table" style="width:100%;margin-bottom:20px"><thead><tr><th>Almacén</th><th>Lote</th><th style="text-align:right">Cant.</th><th></th></tr></thead><tbody>';
